...
class ThemeManager {
  ...
  async fetchCuratedThemes() {
    const results = [];
    for (const item of this.getCuratedList()) {
      try {
        const queries = [`${item.key} wedding decoration`, `${item.key} wedding ceremony`, `${item.key} wedding reception`];
        const seenIds = new Set();
        const images = [];

        for (const q of queries) {
          try {
            const resp = await fetch(`https://api.unsplash.com/search/photos?query=${encodeURIComponent(q)}&per_page=1&client_id=${this.accessKey}`);
            if (!resp.ok) continue;
            const photo = (await resp.json()).results?.[0];
            if (photo && !seenIds.has(photo.id)) { seenIds.add(photo.id); images.push(photo.urls.small); }
          } catch (e) {}
        }

        let link = '#';
        try {
          const resp = await fetch(`https://api.unsplash.com/search/photos?query=${encodeURIComponent(`${item.key} wedding`)}&per_page=1&client_id=${this.accessKey}`);
          if (resp.ok) { const photo = (await resp.json()).results?.[0]; if (photo) link = photo.links.html; }
        } catch (e) {}

        results.push({ key: item.key, title: item.title, description: item.desc, images: images.length ? images : ['assets/img/placeholder.jpg'], link });
      } catch (err) {
        results.push({ key: item.key, title: item.title, description: item.desc, images: ['assets/img/placeholder.jpg'], link: '#' });
      }
    }
    this.themes = results;
  }
  ...
  renderThemes(filter = "") {
    ...
    if (!filtered.length) { this.themeGrid.innerHTML = '<p>No themes found.</p>'; return; }

    filtered.forEach(theme => {
      const card = document.createElement('div');
      card.className = 'theme-card';
      card.innerHTML = `
        <div class="theme-image-grid">${(theme.images || []).map(src => `<img src="${src}" alt="${theme.title}">`).join('')}</div>
      ...
      card.addEventListener('click', () => { this.showThemeExplorer(theme); localStorage.setItem('wedease_selected_theme', theme.title); });
  ...
  showThemeExplorer(theme) {
    ...
    mainImg.src = theme.images?.[0] || 'assets/img/placeholder.jpg';
    ...
    (theme.images || []).forEach((src, idx) => {
      ...
      t.src = src; t.alt = `${theme.title} ${idx + 1}`; t.className = 'theme-modal-thumb';
      t.addEventListener('click', (e) => { e.stopPropagation(); mainImg.src = src; });
    ...
    viewBtn.href = theme.link || '#'; viewBtn.target = '_blank'; viewBtn.rel = 'noopener noreferrer';
    viewBtn.className = 'inspo-btn'; viewBtn.innerText = 'Open on Unsplash';
    actions.appendChild(viewBtn);

    body.appendChild(mainWrap); body.appendChild(thumbs); body.appendChild(actions);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Theme — only on pages that have the theme grid + search
  if (document.querySelector('.theme-grid') && document.getElementById('theme-search')) {
