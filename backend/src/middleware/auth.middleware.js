...
const isDatabaseOrTransientError = (err) => {
  ...
  if (errorName.includes("mongo")) return true;
  if (Array.isArray(err.errorLabels) && err.errorLabels.length > 0) return true;
  ...
  if (typeof errorCode === "number") return true;
...
const authenticate = async (req, res, next) => {
  ...
  const bearerToken = authHeader?.startsWith("Bearer ")
    ? authHeader.split(" ")[1]
    : null;
  const cookieToken = req.cookies?.wedease_token;
